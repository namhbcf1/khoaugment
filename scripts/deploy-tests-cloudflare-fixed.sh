    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('check-health').addEventListener('click', function() {
        ...
        fetch('/api/health')
          ...
          .then(data => {
            const badgeClass = data.status === 'ok' ? 'badge-success' : 'badge-danger';
...
async function handleRequest(request, env) {
  ...
  if (path === '/api/health') {
    return new Response(JSON.stringify({
      ...
      cloudflare: {
        ...
        worker: env.WORKER_NAME || 'khoaugment-integration-tests',
      },
      services: {
        d1: { status: 'connected' },
        r2: { status: 'connected' },
  ...
  if (path === '/api/run-tests' && request.method === 'POST') {
    const body = await request.json();
    const testSuite = body.testSuite || 'all';
...
# Get the worker URL
WORKER_URL=$(wrangler whoami --json | grep -o '"deploymentUrl":"[^"]*"' | cut -d '"' -f 4)

if [ -z "$WORKER_URL" ]; then
  WORKER_URL="https://${TEST_WORKER_NAME}.workers.dev"
  echo -e "${YELLOW}Could not determine worker URL, using default: ${WORKER_URL}${NC}"
else
